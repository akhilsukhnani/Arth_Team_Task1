...
def file_modification_master(input_x):
	if input_x == 1:
		folder_name = input("Enter the same folder name here (eg. dd drive without /) : ")
...
def connect_throw_ssh():
	...
	os.system("ssh root@{} sudo rpm -i /root/arth/src/app/jdk-8u171-linux-x64.rpm".format(ssh_ip))
...
def only_test():
	ssh_ip = input("Enter the IP Address: ")
	os.system("ssh root@{} 'sudo mkdir /root/test; cd /root/test/; touch rahulkumar'".format(ssh_ip))
