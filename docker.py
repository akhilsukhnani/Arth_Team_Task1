...
0 Return to docker menu''')
			...
			if ch5 == 1:
				...
				con_image = input('Enter image to be used, from the locally available images: ')
				...
				file_name = input('Enter the file path to be transfered:(Give absolute path) ')
				con_name = input('Enter container name/id, where you need to transfer file: ')
				con_loc = input('Enter location in continer to be transferd:(Give absolute path) ')
...
0 Return to docker menu ''')
			...
			if ch6 == 1:
				...
				image_name = input('Enter image name(with tag if required):(use format - <IMAGE NAME>:<TAG>' )
